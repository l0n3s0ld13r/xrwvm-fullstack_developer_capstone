...
const Dealer = () => {
  ...
  const get_dealer = async () => {
    try {
      ...
      console.log('Dealer response:', retobj); // Log the response

      if (retobj.status === 200) {
        setDealer(retobj.dealer); // Set the dealer directly
      }
    } catch (error) {
      console.error('Error fetching dealer:', error); // Log any errors
    }
  }
  ...
  const get_reviews = async () => {
    try {
      ...
      const retobj = await res.json();
      console.log('Reviews response:', retobj); // Log the response
